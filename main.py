...
df = pd.read_csv('src/tests/feature_matrix_train.csv', sep=',', header=0)
df.__delitem__('job_id')
x = df.values[:4000, :-1]
y = df.values[:4000, -1]

# start of grid search
GridSearch(RandomForestClassifier(), param_grid, 'f1', n_times=5, k_folds=2, n_top=10,
           bootstrap=False, evaluation_class=None, visualization=True).fit(x, y, threshold)
